...
int socket_connect(git_stream *stream)
{
	...
	if ((ret = p_getaddrinfo(st->host, st->port, &hints, &info)) != 0) {
		char errstr[512];
		sprintf(errstr, "Failed to resolve address for %s: %s", st->host, p_gai_strerror(ret));
		giterr_set_str(GITERR_NET, errstr);
		return -1;
	}
	...
	if (s == INVALID_SOCKET && p == NULL) {
		char errstr[512];
		sprintf(errstr, "Failed to connect to %s", st->host);
		giterr_set_str(GITERR_OS, errstr);
		p_freeaddrinfo(info);
		return -1;
	}
