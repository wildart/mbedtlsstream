		...
		const char *service_suffix)
{
	...
	if (!git__prefixcmp(url, prefix_http)) {
		...
		if (data->use_ssl) {
			giterr_set_str(GITERR_NET, "Redirect from HTTPS to HTTP is not allowed");
			goto cleanup;
		}
	...
	if (!default_port) {
		giterr_set_str(GITERR_NET, "Unrecognized URL prefix");
		goto cleanup;
	}
	...
	if (!error) {
		...
		if (original_host && url[0] != '/' && strcmp(original_host, data->host)) {
			giterr_set_str(GITERR_NET, "Cross host redirect not allowed");
			error = -1;
		}
	}
		...
		const char *default_port)
{
	...
	if (u.field_set & (1 << UF_PATH)) {
		...
		*port = NULL;
		git__free(*host);
		*host = NULL;
		giterr_set_str(GITERR_NET, "invalid url, missing path");
		return GIT_EINVALIDSPEC;
	}
